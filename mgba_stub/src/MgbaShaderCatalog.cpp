...
bool isUnsupportedMgbaShaderType(const std::string& type)
{
...
std::vector<std::string> loadShaderTypesFromFile(const char* path)
{
    ...
    for (const auto& item : parsed)
    {
        ...
        if (type.empty() ||
            isUnsupportedMgbaShaderType(type) ||
...
std::vector<MgbaShaderListEntry> ndsShaderListEntries(const std::vector<std::string>& path)
{
    ...
    if (path.empty())
    {
        ...
        if (hasDrastic)
            result.push_back({MgbaShaderListEntry::Kind::Directory, "DraStic", {}, {"DraStic"}});
    ...
    if (path.size() == 1 && path[0] == "RetroArch")
    {
        ...
        {
            if (isRetroArchShader(type))
                result.push_back({MgbaShaderListEntry::Kind::Shader, ndsShaderDisplayName(type), type, path});
        }
        sortShaderEntries(result);
        return result;
    }
    ...
    if (path.size() == 1 && path[0] == "DraStic")
    {
        ...
        for (const char* category : order)
        {
            if (categories.find(category) != categories.end())
                result.push_back({MgbaShaderListEntry::Kind::Directory, category, {}, {"DraStic", category}});
        }
        sortShaderEntries(result);
        return result;
    }
