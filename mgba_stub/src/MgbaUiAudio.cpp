...
constexpr const char* kSoundRoot = "romfs:/souMgba/switch/";
...
MgbaUiAudioPlayer::MgbaUiAudioPlayer()
{
    Result rc = audoutInitialize();
    if (R_FAILED(rc))
    {
        appendStubLog("GBAStationMgbaStub: ui wav audio audoutInitialize failed rc=%#x", rc);
        return;
    }
    ...
    if (R_FAILED(rc))
    {
        appendStubLog("GBAStationMgbaStub: ui wav audio audoutStartAudioOut rc=%#x", rc);
    }

    m_init = true;
    m_running.store(true, std::memory_order_release);
    m_thread = std::thread(&MgbaUiAudioPlayer::playbackThread, this);
}
...
MgbaUiAudioPlayer::SoundSlot MgbaUiAudioPlayer::slotForMenuSound(MgbaMenuSound sound)
{
    switch (sound)
    {
    case MgbaMenuSound::Focus: return SoundSlot::Focus;
    case MgbaMenuSound::Click: return SoundSlot::Click;
    case MgbaMenuSound::Back: return SoundSlot::Back;
    case MgbaMenuSound::Error: return SoundSlot::Error;
    case MgbaMenuSound::Slider: return SoundSlot::Slider;
    default: return SoundSlot::Error;
    }
}

const char* MgbaUiAudioPlayer::soundFileName(SoundSlot slot)
{
    switch (slot)
    {
...
bool MgbaUiAudioPlayer::loadWav(const char* path, WavData& out)
{
...
bool MgbaUiAudioPlayer::load(SoundSlot slot)
{
    ...
    {
        appendStubLog("GBAStationMgbaStub: ui wav audio load failed path=%s", path);
        return false;
    }
    return true;
}

bool MgbaUiAudioPlayer::play(MgbaMenuSound sound, float pitch)
{
    if (!m_init)
        return false;
...
void MgbaUiAudioPlayer::playbackThread()
{
...
void MgbaUiAudioPlayer::playSoundDirect(const WavData& wav, float pitch)
{
    ...
    if (!rawBuffer)
    {
        appendStubLog("GBAStationMgbaStub: ui wav audio alloc failed bytes=%u",
