...
{
    static const char* labels[] = {"Fit", "Fill", "Original", "4:3", "Integer", "Custom"};
    return labels[std::clamp(index, 0, 5)];
}

const char* integerScaleLabel(int scale)
{
    if (scale <= 0)
        return "auto";
    static char buffer[8];
    std::snprintf(buffer, sizeof(buffer), "x%d", std::clamp(scale, 1, 8));
    return buffer;
}
                     ...
                     float scrollY)
{
    ...
    drawLrSelectorRow(rowPos(y), "画面模式", layoutLabel(layout), contentFocused && focusedRow == 2, true, opacity); y += kSettingStepY;
    drawLrSelectorRow(rowPos(y), "整数倍缩放倍率", integerScaleLabel(integerScaleMultiplier),
                      contentFocused && focusedRow == 3, layout == 4, opacity); y += kSettingStepY;
    drawSubPageRow(rowPos(y), "自定义设置", contentFocused && focusedRow == 4, layout == 5, opacity); y += 65.0f;
                             ...
                             float opacity)
{
    ...
    const float sectionY = portrait ? 158.0f : 116.0f;
    const float rowY = sectionY + 42.0f;
    ...
    Gfx::DrawText(Gfx::SystemFontChinese, {panelX + 30.0f, headerY}, 28.0f,
                  {1.0f, 1.0f, 1.0f, 0.96f * opacity}, "自定义画面设置");
...
void drawTabFrame(MgbaMenuLayer::Item item,
                  ...
                  bool cheatPagePlaceholder,
                  bool displayPagePlaceholder,
                  float offsetY)
{
    ...
    auto drawPage = [&](MgbaMenuLayer::Item page, float offsetX, float opacity) {
        ...
        case MgbaMenuLayer::Item::Display:
            ...
            {
                drawInfoPage("画面设置", "此页面暂未开放。", offsetX, offsetY, opacity);
                break;
            }
        ...
        case MgbaMenuLayer::Item::Cheats:
            if (cheatPagePlaceholder)
            {
                drawInfoPage("金手指设置", "此页面暂未开放。", offsetX, offsetY, opacity);
                break;
            }
