...
- 单屏布局：
  - 非整数倍缩放时按屏幕高度适配并居中；
  - 开启整数倍缩放时按 NDS 原始尺寸 `256x192` 在当前方向下取不出界的最大整数倍并居中。
- `交换上下屏` 热键仍作用于物理屏幕来源，因此单上屏/单下屏也可以通过交换键切换显示来源。
- 完成画面方向渲染：
  - 0 度；
  - 90 度；
  - 180 度；
  - 270 度。
- 方向相关的贴图四角映射按 `GameView::_ndsOrientationUv()` 的等价规则实现，避免旋转后纹理交叉。
- 触摸坐标会按当前方向反算回未旋转的 NDS 下屏坐标。
- `nds.layout.next` 热键循环范围扩展为 8 档。
- 配置读写：
  - `nds.screenLayout` 支持 `top` / `bottom`，兼容 `single_top` / `single_bottom`；
  - `nds.screenOrientation` 使用 `0/90/180/270` 保存。
...
- 绘制变换会作用于：
  ...
  - 字体字形矩形；
  - 菜单内部 scissor 裁剪区域。
- `NdsMenuLayer::draw()` 根据当前 `nds.screenOrientation` 包裹菜单绘制：
  - 0 度：保持原样；
  - 90 度 / 270 度：菜单整体旋转并等比缩放到屏幕内；
  - 180 度：菜单整体翻转。
- 该实现只影响菜单绘制阶段，不改变游戏层当前稳定的手动旋转逻辑，也不改 `Gfx::EndFrame(..., 0)` 的全局提交路径。
...
```text
GBAStation.nro        24.92 MB
GBAStationNDSStub.nro 2.28 MB
```

---

## 阶段 4.13：修正 90/270 菜单方向与无缩放自适应

### 已实施

- 修正菜单 90 度 / 270 度旋转方向：
  - 之前菜单方向与游戏画面方向相反；
  - 现在 90/270 的菜单旋转矩阵已反向调整，与游戏画面方向一致。
- 移除菜单整体等比缩放：
  - 不再把 1280x720 菜单压缩后旋转；
  - 字体绘制尺寸保持原始字号，不随方向缩放。
- 增加菜单 UI metrics：
  - 横屏逻辑画布：1280x720；
  - 90/270 逻辑画布：720x1280；
  - 菜单绘制前按方向选择 metrics，绘制后恢复横屏 metrics，避免污染游戏层状态徽标。
- 90/270 自适应布局：
  - 左侧菜单、内容区、分割线、底栏、删除对话框按 720x1280 逻辑尺寸重算；
  - 保存/读取状态页在竖向逻辑画布下自动改为 1 列；
  - 内容滚动高度按当前方向重新计算。
...
## 阶段 4.17：修正混合横向布局左右间距

### 目标

- `HybridHorizontal` 布局中，左侧主屏与右侧双屏堆叠区之间的实际距离也必须由 `屏幕间距` 控制。

### 已实施

- 调整 `HybridHorizontal` 右侧堆叠屏的水平对齐：
  - 右侧上屏从水平居中改为靠左；
  - 右侧下屏从水平居中改为靠左。
- 保持左侧主屏靠右：
  - 左侧主屏右边缘；
  - 右侧堆叠屏左边缘；
  - 两者之间的距离现在直接由 `nds.screenGap` 控制。
- 该改动同时作用于：
  - 非整数倍缩放；
  - 整数倍缩放。
...
- 将 `屏幕间距` 从固定档位 LR 选择器改为可复用的数值调整器；
- 数值调整器支持：
  - 默认值；
  - 数据步长；
  - LR 加减；
...
- 新增菜单绘制控件：
  - `drawNumberAdjusterRow(...)`；
  - 显示当前数值、默认值、步长和 LR 按键提示。
- `屏幕间距` 改为数值调整器：
  - 默认值：`0px`；
  - 步长：`2px`；
  - 范围：`-64px ~ 64px`；
  - L 减小，R 增大；
  - 按 A 恢复默认值。
- `NdsMenuLayer` 移除固定数组档位：
  - 不再使用 `0 / 2 / 4 / 8 / ...` 的离散列表；
  - 改为步进式数值计算。
- `NdsGameLayer::setScreenGap()` 支持负数：
  - 正数表示屏幕间距拉开；
  - 负数表示两个屏幕内侧轻微重叠。
- 布局计算增加 `clampGap()`：
  - 对正负 gap 都做安全夹取；
...
## 阶段 4.15：屏幕内侧贴边与可配置屏幕间距

### 目标

- 双屏布局不再让上屏/下屏分别在各自区域内居中导致中间距离过大；
- 左右布局时：左侧屏幕靠右、右侧屏幕靠左；
- 上下布局时：上屏靠下、下屏靠上；
- 两个屏幕默认紧挨，并允许在画面设置中用 LR 调整间距。

### 已实施

- `NdsGameLayer` 新增 `screenGap` 布局参数：
  - 默认 `0px`；
  - 当前可选值：`0 / 2 / 4 / 8 / 12 / 16 / 24 / 32 / 48 / 64px`。
- 新增对齐版缩放函数：
  - `fitAspectAligned(region, alignX, alignY)`；
  - `fitIntegerAligned(region, scale, alignX, alignY)`。
- 调整双屏布局对齐规则：
  - `Horizontal`：左侧上屏右贴，右侧下屏左贴；
  - `Vertical`：上屏下贴，下屏上贴；
  - `TopPriority` / `BottomPriority`：大屏和小屏按内侧边缘相邻；
  - `HybridHorizontal`：左侧主屏右贴，右侧上下堆叠屏按内侧边缘贴合。
- `NdsDisplaySettings` 新增 `screenGap` 字段。
- 画面设置页新增 LR 选择器：
  - 名称：`屏幕间距`；
  - 切换后立即应用到游戏画面。
- 配置读写新增：
  - `nds.screenGap`。
- 修正画面设置页滚动焦点上限，避免新增设置项后底部项目滚动不到位。
...
## 阶段 4.14：修复混合横向布局权重计算黑屏

### 现象

- `HybridHorizontal` 在未开启整数倍缩放时全黑，看不到游戏画面。

### 原因

- `10/3` 和 `5/3` 应作为左右区域的宽度权重比例使用；
- 当前代码直接执行 `bounds.w * 10/3` 和 `bounds.w * 5/3`，导致左右区域总宽变成 `5 * bounds.w`；
- `fitAspect()` 在超大区域内居中后，实际绘制矩形被推到屏幕外，最终画面全黑。

### 已实施

- 将 `10/3 : 5/3` 改为归一化权重：
  - 左侧区域宽度 = `bounds.w * (10/3) / (10/3 + 5/3)`，即 `2/3` 屏幕宽度；
  - 右侧区域宽度 = 剩余 `1/3` 屏幕宽度；
  - 右侧继续上下拆分为上屏/下屏。
- 保留用户调整后的比例意图，不回退到旧的 `0.7 / 0.3`。
