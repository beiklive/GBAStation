...
#define uvratio (InputSize/TextureSize)//added by Roc-Y


//#define SourceSize vec4(TextureSize, 1.0 / TextureSize) //either TextureSize or InputSize
#define SourceSize vec4(InputSize, 1.0 / InputSize) //Fixed by Roc-Y
#define outsize vec4(OutputSize, 1.0 / OutputSize)

vec3 FsrEasuCF(vec2 p) {
    return COMPAT_TEXTURE(Source,p *uvratio).rgb;//*uvratio added by Roc-Y
}
...
void main()
{
    ...
    vec2 vTexCoord = TEX0.xy/uvratio;//added by Roc-Y

    vec2 fragCoord = vTexCoord.xy * OutputSize.xy;
    fragCoord = floor(fragCoord);//fixed by Roc-Y
