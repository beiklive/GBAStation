...
#define BK_FORWARDER_CONCAT_INNER(a, b) a##b
#define BK_FORWARDER_CONCAT(a, b) BK_FORWARDER_CONCAT_INNER(a, b)
#define R_TRY(expression) do { const Result rc_ = (expression); if (R_FAILED(rc_)) return rc_; } while (0)
#define R_UNLESS(condition, result) do { if (!(condition)) return (result); } while (0)
#define R_SUCCEED() return 0
#define ON_SCOPE_EXIT(...) \
    auto BK_FORWARDER_CONCAT(scopeExit_, __LINE__) = makeScopeExit([&]() { __VA_ARGS__; })
...
Result initializeHeaderKey(ForwarderKeys& keys)
{
    u8 headerKek[0x20]{};
    R_TRY(splCryptoGenerateAesKek(HEADER_KEK_SRC, 0, 0, headerKek));
    R_TRY(splCryptoGenerateAesKey(headerKek, HEADER_KEY_SRC, keys.header_key));
    R_TRY(splCryptoGenerateAesKey(headerKek, HEADER_KEY_SRC + 0x10, keys.header_key + 0x10));
    R_SUCCEED();
}
...
Result initializeNs()
{
    ...
    if (hosversionAtLeast(3, 0, 0))
        return nsGetApplicationManagerInterface(&g_nsAppSrv);
...
{
    const struct { u8 last_modified_event; u8 padding[7]; u64 tid; } in = {3, {0}, tid};
...
void write_nca_header_encypted(nca::Header& nca_header, u64 tid, const ForwarderKeys& keys, nca::ContentType type, BufHelper& buf) {
...
auto create_program_nca(u64 tid, const ForwarderKeys& keys, const FileEntries& exefs, const FileEntries& romfs, const FileEntries& logo) -> NcaEntry {
    BufHelper buf;
...
auto create_control_nca(u64 tid, const ForwarderKeys& keys, const FileEntries& romfs) -> NcaEntry{
    nca::Header nca_header{};
    BufHelper buf;
    buf.write(&nca_header, sizeof(nca_header));
...
auto create_meta_nca(u64 tid, const ForwarderKeys& keys, NcmStorageId storage_id, const std::vector<NcaEntry>& ncas) -> NcaMetaEntry {
...
auto install_forwarder_internal(OwoConfig& config, NcmStorageId storage_id) -> Result {

    R_UNLESS(!config.nro_path.empty(), kResultBadArgs);
    R_UNLESS(!config.icon.empty(), kResultBadArgs);
    ...
    R_TRY(initializeNs());
    ON_SCOPE_EXIT(exitNs());

    ForwarderKeys keys;
    R_TRY(initializeHeaderKey(keys));
    ...
    // push record
    {
        ...
        R_TRY(pushApplicationRecord(tid, &content_storage_record, 1));
