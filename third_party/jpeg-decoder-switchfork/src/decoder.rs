...
impl<R: Read> Decoder<R> {
    ...
    ///
    /// SWITCHFORK: always pick Immediate so jpeg_decoder never spawns a
    /// std::thread. Switch's newlib pthread shim crashes on thread spawn
    /// inside decode for Mario-63-sized JPEGs (250x256 above the original
    /// 128*128 threshold). The Immediate worker is a single-threaded
    /// decoder; for our use case the perf cost is negligible and we don't
    /// crash.
