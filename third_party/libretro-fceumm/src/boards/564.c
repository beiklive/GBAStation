...
static void trapLatchWrite (uint16_t *newAddress, uint8_t *newValue, uint8_t romValue) { /* After the lock bit is set, only update the (mode-specific) inner bank bits. */
	if (Latch_data &0x20) {
		if (Latch_data &0x08)
