 ...
 * Newer GCC treats calls through an empty-parameter prototype as strict
 * no-argument calls, while the reentrant scanner receives parser state and
 * scanner context.  Keep the generated parser source buildable without
 * regenerating it with the host bison/flex versions.
 */
extern int libconfig_yylex(void *yylval_param, void *yyscanner);
extern int libconfig_yyget_lineno(void *yyscanner);
